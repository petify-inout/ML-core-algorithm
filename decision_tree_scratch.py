...
class DecisionTree:
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        best_feat = self._best_criteria(X, y, feat_idxs)
        
        # grow the children that result from the split
        left_idxs, right_idxs = self._split(X[:, best_feat])
    ...
    def _information_gain(self, y, X_column):
        ...
        # generate split
        left_idxs, right_idxs = self._split(X_column)
    ...
    def _split(self, X_column):
        left_idxs = np.argwhere(X_column == 0).flatten()
        right_idxs = np.argwhere(X_column == 1).flatten()
